...
local function main()
    ...
    if sel == "both" then
        if GetMediaItemInfo_Value(ctx.item1, "D_FADEOUTLEN") - amt < min_fade then
            ...
            PreventUIRefresh(-1); Undo_EndBlock("Xfade Shrink Fade End", -1); return
        end
    ...
    elseif sel == "left" then
        ...
        say("Fade-in end shrunk")
    end

    UpdateArrange()
    UpdateTimeline()
    PreventUIRefresh(-1)
    Undo_EndBlock("Xfade Shrink Fade End", -1)
end
