...
local create_dest_in, return_xfade_length, xfade, get_first_last_item, SDmarkers
...
function Main()
  ...
  if SDmarkers() == 4 then
    ...
    local state = r.GetToggleCommandState(1156)
    if state == 1 then
      r.Main_OnCommand(1156,0) -- Options: Toggle item grouping and track media/razor edit grouping
    end
    r.Main_OnCommand(42398, 0) -- Item: Paste items/tracks
    local xfade_len = return_xfade_length()
    r.GoToMarker(0, 996, false)
    r.MoveEditCursor(-xfade_len, false) -- move cursor back xfade length
    r.Main_OnCommand(40625, 0) -- Time Selection: Set start point
    ...
    r.Main_OnCommand(40289, 0) -- Item: Unselect all items 
    r.MoveEditCursor(xfade_len, false) -- move cursor forward xfade length
