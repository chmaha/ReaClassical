...
function main()
    ...
    if visible then
        ...
        local changed_unit, new_unit = ImGui.Combo(ctx, "##unit_type",
            unit_type, table.concat(unit_names, "\0") .. "\0")
        ...
        ImGui.SetNextItemWidth(ctx, -1)
        local changed_format, new_format = ImGui.Combo(ctx, "##format", format_type,
            table.concat(format_names, "\0") .. "\0")
        ...
        if format_type == 1 then
            ...
            ImGui.SetNextItemWidth(ctx, -1)
            local changed_bitrate, new_bitrate = ImGui.Combo(ctx, "##bitrate", bitrate,
                table.concat(bitrate_names, "\0") .. "\0")
        ...
        local changed_sr, new_sr = ImGui.Combo(ctx, "##sample_rate", sample_rate,
            table.concat(sample_rate_names, "\0") .. "\0")
        if changed_sr then
            sample_rate = new_sr
        end
        if format_type == 1 then
        ...
        local changed_bd, new_bd = ImGui.Combo(ctx, "##bit_depth", bit_depth,
            table.concat(bit_depth_names, "\0") .. "\0")
        if changed_bd then
            bit_depth = new_bd
        end
        if format_type == 1 then
