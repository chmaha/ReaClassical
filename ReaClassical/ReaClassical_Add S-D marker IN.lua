...
local move_destination_folder, get_tracks_per_group, calculate_destination_info
...
function main()
    ...
    if cur_pos ~= -1 then
        local is_destination = (workflow == "Horizontal")
        if not is_destination then
            local resolved_track = track or selected_track
            is_destination = (get_track_prefix(resolved_track) == "D")
        end

        if is_destination then
            add_destination_in(cur_pos, workflow, selected_track)
        else
            add_source_in(cur_pos, workflow, track, selected_track, moveable_dest, to_takemarkers)
        end
    end
    PreventUIRefresh(-1)
end
...
function add_destination_in(cur_pos, workflow, selected_track)
    local i = 0
    while true do
    ...
    local marker_color = (workflow == "Horizontal") and colors.dest_marker or colors.dest_items
...
function add_source_in(cur_pos, workflow, track, selected_track, moveable_dest, to_takemarkers)
