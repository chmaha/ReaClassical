...
function markers()
    ...
    local raw_labels = {}  -- [marker_id] = raw label string
    ...
    if proj_marker_count == 0 then
        ...
        -- Derive track numbers from marker labels to correct stale ProjExtState values
...
function clean_up(is_selected, dest_in, dest_out, source_in, source_out)
...
function reposition_dest_out(dest_workflow)
    ...
    for i = 0, num_markers + num_regions - 1 do
        ...
        if id == 997 then
            dest_out_name = name; dest_out_color = color; break
        end
    end
...
function reposition_dest_in(dest_workflow)
    ...
    for i = 0, num_markers + num_regions - 1 do
        local _, _, _, _, name, id, color = EnumProjectMarkers3(0, i)
        if id == 996 then
            dest_in_name = name; dest_in_color = color; break
        end
    end
