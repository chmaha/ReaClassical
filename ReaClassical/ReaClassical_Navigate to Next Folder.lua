...
local main, is_special_track, get_folder_parent, get_rc_folders, solo, say
...
function main()
    ...
    local next_pos = current_pos and (current_pos + 1) or 1

    if next_pos > #folders then
        say("At bottom of project.")
        return
    end

    local next_folder = folders[next_pos]
    PreventUIRefresh(1)
    SetOnlyTrackSelected(next_folder.track)
    solo()
    PreventUIRefresh(-1)
    TrackList_AdjustWindows(false)
    UpdateArrange()
    UpdateTimeline()
    local _, name = GetSetMediaTrackInfo_String(next_folder.track, "P_NAME", "", false)
    say("Folder: " .. name)
end
