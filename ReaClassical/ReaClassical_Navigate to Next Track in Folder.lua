...
function main()
    ...
    if is_special_track(selected) then
        ...
        if current_pos >= #specials then
            say("Last special track")
            return
        end
        clear_solo_if_special(specials[current_pos].track, specials[current_pos].pext)
        local next_s = specials[current_pos + 1]
        SetOnlyTrackSelected(next_s.track)
        TrackList_AdjustWindows(false)
        apply_solo_for_special(next_s.track, next_s.pext)
        announce_special_track(next_s.track, next_s.pext)
        return
    end
