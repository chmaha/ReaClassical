...
function main()
    ...
    if cur_pos ~= -1 then
        ...
        if is_destination then
            add_destination_out(cur_pos, workflow, selected_track)
        else
            add_source_out(cur_pos, workflow, track, selected_track, to_takemarkers)
        end
    end
    PreventUIRefresh(-1)
end
...
function add_destination_out(cur_pos, workflow, selected_track)
    local i = 0
    while true do
...
function add_source_out(cur_pos, workflow, track, selected_track, to_takemarkers)
