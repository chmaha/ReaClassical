...
-- Monitor playback and stop at SAO marker
...
-- Play from marker (selects parent track based on marker number)
...
-- Convert action: move to marker and run command
function ConvertAtMarker(pos, marker_name)
    SetEditCurPos(pos, true, true)
...
function Loop()
    ...
    visible, open = ImGui.Begin(ctx, 'SAI Marker Manager', true, window_flags)
    ...
    if visible then
        ...
        local markers = GetSAIMarkers()

        if #markers == 0 then
            ImGui.Text(ctx, "No SAI markers found in project...")
        else
            -- Create table
            if ImGui.BeginTable(ctx, 'MarkerTable', 6, ImGui.TableFlags_Borders | ImGui.TableFlags_RowBg) then
                -- Setup columns
                ...
                for i, marker in ipairs(markers) do
                    ...
                    if ImGui.Button(ctx, '⚡##convert' .. marker.marker_num, 40, 0) then
                        ConvertAtMarker(marker.pos, marker.name)
                    end
                end
