...
function main()
    ...
    local csv = "Edit,Start,Source Filename,Source In,Source Out,End Check,Playback Rate\n"
    ...
    file:write([[
        ...
        <style>
            ...
            .container { width: 75%; margin: auto; text-align: left; }
    ...
    file:write("  <p><strong>Frame Rate:</strong> <span id='fpsValue'>", (fps or "25"), "</span> fps</p>\n")
    file:write([[<p><span class="label">Start:</span>
    ...
    for _, item in ipairs(items) do
        if item.edit_number == "Gap" then
            ...
            file:write(string.format(
                ...
                '<td>%s</td><td>%s</td></tr>\n',
                item.edit_number, item.position, item.filename,
                item.s_in, item.s_out, item.dest_end, item.playrate
            ))
        end
    end
...
function get_html_file()
    ...
    local file = path .. slash .. 'edit_list_sft.html'
    return file
end
