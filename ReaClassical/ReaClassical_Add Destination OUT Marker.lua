...
function main()
    ...
    if edge_check(cur_pos) == true then
        local response = ShowMessageBox("The marker you are trying to add would either be on or close to an item edge or crossfade. Continue?", "Add Dest-OUT Marker", 4)
...
function edge_check(cur_pos)
    ...
    for i = 0, num_of_items - 1 do
        ...
        if cur_pos > item_start - 0.5 and cur_pos < item_fadein_end then 
            clash = true
            break 
        end
