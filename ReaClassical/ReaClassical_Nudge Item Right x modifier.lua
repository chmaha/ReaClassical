...
function main()
    if xfu.is_xfade_mode() then
        say("Use XFM Nudge Right in crossfade mode")
        return
    end
    ...
    if moved == 0 then
        ...
        say(moved .. " item group" .. (moved ~= 1 and "s" or "") .. " nudged " .. ms .. " milliseconds right")
    end
end
...
function nudge_items(amount)
    ...
    UpdateTimeline()
    PreventUIRefresh(-1)
    Undo_EndBlock("Nudge items right x modifier", -1)
    return group_count
end
