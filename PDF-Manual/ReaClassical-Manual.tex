%% LyX 2.4.3 created this file.  For more info, see https://www.lyx.org/. % Do
...
Pressing \keys{F5} brings up the ReaClassical Preferences dialog. The first line
...
hovering of the right-hand items of a crossfaded pair and pressing \keys{F}.
Finally, you can set the custom playback rate when auditioning via
\keys{\shift+A}. There will be more on these settings in subsequent sections of
...
function at any time via \keys{\ctrl+F9}.
...
\keys{F8}, add a REF track \keys{'} and set the ReaClassical Preferences option
`REF = Overdub Guide' to `1' (\keys{F5}). With this setting, audio on the guide
track will be audible during take recording and auditioning of folders. Use
Classical Take Recording \keys{F9} and Take Counter window \keys{\ctrl+\enter}
...
track in the destination folder \keys{\shift+T} (see below).
...
Project Markers (\keys{\shift+del}), and finally ReaClassical Help (\keys{H}).
...
via ReaClassical Preferences (\keys{F5}). In practice this means that if you
...
on the destination parent track, run \keys{\ctrl+T}. Track titles are then added
to the individual items on the destination parent track by double-clicking on
the item or selecting and pressing \keys{F2}. Note that only items that start
...
title, performer, composer and genre. The Markers to CUE function \keys{C} will
...
A CUE file can be generated via \keys{C} (no need create a time selection). The
...
When using the \keys{C} shortcut, ReaClassical also generates both a plain text
...
without leaving my favorite DAW. However, the Marker to CUE function \keys{C} is
...
is set to 1 in ReaClassical Preferences \keys{F5}). The user is then presented
