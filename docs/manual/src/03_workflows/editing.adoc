...
The Prepare Takes function is essential when working with inherited or pre-edited projects. Some projects also arrive with items that have been individually top-and-tailed to remove silence. In this case, items may look aligned musically, but standard REAPER no longer understands them as belonging together. Prepare Takes fixes this by ensuring all related audio behaves as a single logical unit when dragging, adjusting item starts, ends fades etc. In addition, certain advanced ReaClassical functions like healing edits, rely on correct item grouping.
...
TIP: Prepare Takes will automatically run on placing the first S-D marker in the project.
