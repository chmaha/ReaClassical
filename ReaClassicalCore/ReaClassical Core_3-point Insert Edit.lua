...
local main, markers, copy_source, split_at_dest_in
...
local save_last_assembly_item, select_items_containing_midpoint
local get_item_by_guid, add_marker
...
function main()
        ...
        if last_saved_item then
            ...
            if math.abs(item_right_edge - pos_table[1]) > threshold then
                ...
                elseif user_input == 7 then
                    ...
                    add_marker(item_right_edge, 0, "DEST-IN", 996, 0)
                end
            end
        end
        add_marker(pos_table[1], 0, "DEST-OUT", 997, 0)
    else
        MB(
    ...
    if new_dest_count + new_source_count == 4 then -- final check we actually have 4 S-D markers
        ...
        add_marker(end_of_new_item, 0, "DEST-IN", 996, dest_marker)
...
function add_marker(pos, distance, label, num, color)
    DeleteProjectMarker(nil, num, false)
    AddProjectMarker2(0, false, pos + distance, 0, label, num, color)
end
